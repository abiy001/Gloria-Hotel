...
<head>
  ...
  <title>Dashboard - Manage City</title>
...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1 class="text-black">Data Kota</h1>
    </div><!-- End Page Title -->
...
<div class="card">
        ...
        <img src=" {{ $item -> city_image  }}" alt="" class="w-full h-[250px]">
        <div class="card-body" >
            <h5 class="card-title ">{{ $item -> city_name }}</h5>

            <form id="form_delete" action="{{ route('city.delete',  $item -> id ) }}" method="POST">
              @csrf
              @method('delete')
...
<div class="static h-screen w-full ">    <div class="fixed bottom-0 right-2 h-16 w-16 ">
    <a href="{{ route('addCity.dashboard.view') }}" class="bg-dark text-white rounded-lg px-4 py-3 "><i class="bi bi-plus"></i></a>
  </div>
</div>
